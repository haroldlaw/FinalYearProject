...
const Profile = () => {
  ...
  const fetchUploadHistory = async () => {
    try {
      setLoading(true);
      // Replace with actual API call to get user's upload history
      // const response = await imageAPI.getUserHistory();
      
      // Mock data for demonstration
      const mockHistory = [
        {
          id: 1,
          imageUrl: "https://via.placeholder.com/300x200/4A90E2/FFFFFF?text=Sample+1",
        ...
        {
          id: 2,
          imageUrl: "https://via.placeholder.com/300x200/7ED321/FFFFFF?text=Sample+2",
        ...
        {
          id: 3,
          imageUrl: "https://via.placeholder.com/300x200/F5A623/FFFFFF?text=Sample+3",
  ...
  const getScoreBadge = (score) => {
    if (score >= 85) return "bg-green-500/30 text-green-300 border-green-400/50";
    if (score >= 70) return "bg-yellow-500/30 text-yellow-300 border-yellow-400/50";
      ...
      >
        <div className="bg-black/50 backdrop-blur-xl border-2 border-white/40 rounded-3xl shadow-2xl p-10">
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-3">Loading Profile...</h2>
          </div>
        </div>
      </div>
    );
      ...
      <div className="w-full max-w-7xl mx-auto px-6 py-6 min-h-full">
        ...
        <div className="bg-black/60 backdrop-blur-xl border-2 border-white/40 rounded-3xl p-6 mb-6 shadow-2xl hover:shadow-blue-500/30 transition-all duration-500">
          <div className="flex justify-between items-center">
            ...
            <button
              onClick={() => navigate("/")}
              className="bg-linear-to-r from-blue-500/90 to-green-500/90 backdrop-blur-sm border-2 border-white/30 text-white px-6 py-3 rounded-2xl hover:from-blue-600/90 hover:to-green-600/90 transition-all duration-300 font-semibold shadow-lg hover:shadow-green-500/30 hover:scale-105"
            >
              🏠 Home
            </button>
          </div>
        ...
        <div className="bg-black/50 backdrop-blur-xl border-2 border-white/40 rounded-3xl shadow-2xl p-6 mb-8 hover:shadow-purple-500/30 transition-all duration-500">
          <div className="grid md:grid-cols-4 gap-6">
            ...
            <div className="text-center">
              ...
              <p className="text-white font-semibold text-xl">{user.totalUploads}</p>
            </div>
            <div className="text-center">
              <div className="text-2xl mb-2">⭐</div>
              ...
              <p className="text-white font-semibold text-xl">
                {uploadHistory.length > 0 
                  ? Math.round(uploadHistory.reduce((sum, img) => sum + img.analysisScore, 0) / uploadHistory.length)
                  : "N/A"
                }
              </p>
            </div>
        ...
        <div className="bg-black/50 backdrop-blur-xl border-2 border-white/40 rounded-3xl shadow-2xl p-6 hover:shadow-green-500/30 transition-all duration-500">
          ...
          {uploadHistory.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-white text-xl font-bold mb-2">No uploads yet</h3>
              <p className="text-white/70 mb-6">Start by uploading your first image!</p>
              <button
                onClick={() => navigate("/")}
              ...
              {uploadHistory.map((image) => (
                ...
                >
                  ...
                  <div className="space-y-2">
                    <h4 className="text-white font-semibold truncate">{image.originalName}</h4>
                    
                    <div className="flex justify-between items-center">
                      <span className={`text-2xl font-bold ${getScoreColor(image.analysisScore)}`}>
                        {image.analysisScore}/100
                      </span>
                      <span className={`text-xs px-2 py-1 rounded-full border ${getScoreBadge(image.analysisScore)}`}>
                        {image.analysisScore >= 85 ? "Excellent" : image.analysisScore >= 70 ? "Good" : "Fair"}
                      </span>
                    </div>
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-6">
            <div className="bg-black/70 backdrop-blur-xl border-2 border-white/40 rounded-3xl p-6 max-w-4xl max-h-[90vh] overflow-auto">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-2xl font-bold text-white">📷 {selectedImage.originalName}</h3>
                ...
                <div className="space-y-4">
                  <div className="bg-gray-900/50 rounded-2xl p-4 border border-white/20">
                    ...
                    <div className="flex items-center space-x-3">
                      <span className={`text-3xl font-bold ${getScoreColor(selectedImage.analysisScore)}`}>
                        {selectedImage.analysisScore}/100
                      </span>
                      <span className={`text-sm px-3 py-1 rounded-full border ${getScoreBadge(selectedImage.analysisScore)}`}>
                        {selectedImage.analysisScore >= 85 ? "Excellent" : selectedImage.analysisScore >= 70 ? "Good" : "Fair"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-900/50 rounded-2xl p-4 border border-white/20">
                    ...
                    <div className="space-y-2 text-white/90">
                      <p><span className="font-semibold">📁 File:</span> {selectedImage.originalName}</p>
                      <p><span className="font-semibold">📏 Size:</span> {(selectedImage.fileSize / 1024 / 1024).toFixed(2)} MB</p>
                      <p><span className="font-semibold">📅 Uploaded:</span> {formatDate(selectedImage.uploadDate)}</p>
                      <p><span className="font-semibold">✅ Status:</span> <span className="capitalize">{selectedImage.status}</span></p>
                    </div>
                  </div>
                </div>
              </div>
